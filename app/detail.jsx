/* ============================================================
   Numberline app - Contact view (exports to window)
...
function CallLaunch({ target, devices, numbers, onClose }){
  ...
  const opts = devices.filter(d=>d.enabled!==false).map(d=>{
    ...
    return { id:d.id, t:'Numberline app', s:d.user, icon:'smartphone' };
  });
    ...
    return (
      <div className="cl-pop" onMouseDown={e=>e.stopPropagation()}>
        <div className="cl-ring">
          ...
          <p>Pick up {ringing.s} and Numberline will connect you to {toNum}.</p>
  ...
  return (
    <div className="cl-pop" onMouseDown={e=>e.stopPropagation()}>
      <div className="cl-h"><b>Call {target}</b><p>Numberline rings one of your devices first, then connects the call.</p></div>
      {callNums.length>1 && (
