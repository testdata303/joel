/* ============================================================
   Numberline - Settings sections, part 1: shared helpers + System + Team
...
function DirectorySection({ extensions, onBack }){
  ...
  return (
    <SetDetail title="Directory" onBack={onBack}
      sub="Callers press 9 to reach a teammate by name instead of remembering an extension number. Numberline builds the directory automatically from your extension names.">
      ...
      {on && <>
        <Card icon="mic" title="How callers search" flush>
          <SRow title="Say a name" meta="“Sales,” “Dr. Niaraki,” “Bob” - Numberline listens and connects."
        ...
        <Card icon="layers" title="In the directory" desc="Choose who callers can reach by name. Numberline uses each extension’s spoken (text-to-voice) name." flush>
          {roster.map(e=>{
...
const DESK_MODELS = [
  { id:'yealink-t31g', brand:'Yealink', name:'T31G', tag:'Our pick', price:'~$65',
    blurb:'The desk phone we recommend - reliable, inexpensive, and plug-and-play with Numberline. A sharp display and gigabit passthrough.',
...
function AddDeskPhoneFlow({ onClose, onAdd }){
  ...
  const code='Numberline-4F2A-9C71';
    ...
    <Modal wide title="Add a desk phone" icon="monitor" onClose={onClose} footer={footer}>
      ...
      {step===1 && (
        <div className="dp-learn">
          <p className="dp-lead">Prefer a real handset on the desk? Numberline works with standard SIP desk phones - bring your own and point it at an extension, just like the mobile app. Here’s how it works:</p>
...
function DevicesSection({ onBack }){
  ...
  return (
    <SetDetail title="Devices" onBack={onBack}
      ...
      <p className="ss-foot">Numberline supports approved phone models only - plug one in and it connects automatically. No setup codes, no telecom settings.</p>
      ...
      <p className="ss-foot">A teammate’s phone becomes a device when they sign in to the Numberline app. Add people on the <b>Users &amp; roles</b> screen.</p>
    </SetDetail>
  );
}
...
function HoldSection({ onBack }){
  ...
  return (
    <SetDetail title="Music on hold" onBack={onBack}
      sub="What callers hear while they wait to be connected - for example during call screening, or while Numberline rings your next destination.">
      <Card flush>
        {SX_HOLD.map(h=>(
...
function UsersSection({ onBack, extensions }){
  ...
  return (
    <SetDetail title="Users & roles" onBack={onBack}
      ...
      <p className="ss-foot">Numberline still rings your phones whether or not anyone has an account - accounts only control who can sign in and what they can see or change. There’s always exactly one <b>Owner</b>: full access including billing, can’t be removed.</p>
    </SetDetail>
  );
}
  ...
  return (
    <SetDetail title="Business profile" onBack={onBack}
      ...
      <Card icon="building" title="Identity">
        ...
        <Field label="How it’s pronounced" help="How Numberline says your name in generated audio. Spell it out if it’s tricky - e.g. “Smile-bar.”">
      ...
      <Card icon="clock" title="Time & hours" flush>
        <div className="ss-row">
          ...
          <span className="ss-main"><b>Time zone</b><span className="ss-meta">Numberline uses this to decide when you’re open and which greeting to play.</span></span>
