/* ============================================================
   Numberline app - System page (exports to window)
...
function NumberCard({ n, extensions, businessName, onRouting, onDelete, onGoGreetings, onGoExtensions }){
  ...
  const tip = isMenu ? 'Callers hear your main greeting, then the phone menu.'
    ...
    : TE==='music' ? 'Hold music plays while Numberline rings your destinations.'
    : 'Standard ringing plays while Numberline rings your destinations.';
          ...
          <Card icon="route" title="After the greeting" desc="What callers can do once they’ve heard it.">
            <div className="choices">
              <Choice on={flow==='operator'} onClick={()=>setFlow('operator')}
                ...
                desc={`Callers can press a key, or just stay on the line - Numberline connects them to ${opName} (Ext 0). The simplest, most forgiving setup, and the default.`}/>
              <Choice on={flow==='require'} onClick={()=>setFlow('require')}
                title="Require callers to choose"
                desc="Callers must press a key. If they stay silent, Numberline replays the greeting once, then connects them to the operator so no one is stranded."/>
            </div>
