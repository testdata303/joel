/* ============================================================
   Numberline - Settings sections, part 2: Messaging/compliance + Advanced
...
function SmsComplianceSection({ numbers, onBack }){
  ...
  return (
    <S2Detail title="SMS compliance" onBack={onBack}
      sub="Before a business number can text, carriers require it to be registered. Numberline files the registration for you - you just confirm a few details once.">
      ...
      <p className="ss-foot">Toll-free numbers and local numbers register differently, but Numberline handles both. Review usually takes 1–3 business days; calling keeps working the whole time.</p>
    </S2Detail>
  );
}
...
function RetentionSection({ onBack }){
  ...
  return (
    <S2Detail title="Data retention" onBack={onBack}
      sub="How long Numberline keeps your call recordings, voicemails, and messages before clearing them out automatically.">
      <div className="note info" style={{marginBottom:18}}>
        <S2Icon name="info"/>
        <span>When a recording or voicemail reaches its limit, Numberline deletes the <b>audio</b> but keeps the <b>transcript, summary, and call details</b>. You keep the searchable history without paying to store the audio forever.</span>
      </div>
      <S2Card flush>
...
function IntegrationsSettingsSection({ onBack }){
  ...
  return (
    <S2Detail title="Integrations" onBack={onBack}
      sub="Send Numberline activity to the tools your team already uses. Integrations belong to the business - admins set them up once for everyone.">
      ...
      <div className="note info" style={{marginBottom:14}}>
        <S2Icon name="shield"/>
        <span>Only admins add channels here. Everywhere notifications are set - on an extension or the shared inbox - your team <b>picks from this list</b>. Teammates can’t point Numberline at their own channels, so activity only ever flows where you’ve approved it.</span>
      </div>
      <S2Card flush>
        {channels.map(c=>(
      ...
      <S2Card flush>
        <S2Row icon="appbell" iconCls="blue" title="In-app" meta="Everyone with access sees assigned activity in their Numberline inbox."
        ...
        <S2Row icon="slack" title="Slack" meta="Shared, admin-curated channels only - Numberline never sends Slack DMs to individuals."
      ...
      <p className="ss-foot">Webhooks send a small message to a URL you control whenever an event happens. If a webhook keeps failing, Numberline flags it as <b>Needs attention</b> and keeps retrying.</p>
    </S2Detail>
  );
}
...
function NumberAdvSection({ numbers, onNav, onBack }){
  ...
  return (
    <S2Detail title="Advanced number options" onBack={onBack}
      ...
      <S2Card flush>
        <S2Row icon="forward" title="Bring a number to Numberline" meta="Move an existing business number from another carrier. Calls keep working while it transfers." tappable onClick={()=>onNav&&onNav('numbers')}
