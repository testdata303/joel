/* ============================================================
   ...
   ============================================================ */
const { Icon, Toggle, Modal, Segmented, Card } = window;
...
function NumberCard({ n, extensions, businessName, onRouting, onDelete, onGoGreetings, onGoExtensions }){
  ...
  const sms = n.sms ? (SMS_STATUS[n.sms] || SMS_STATUS.approved) : null;
...
function GreetingsScreen({ onGoExtensions, extensions, businessName }){
  ...
  const mainSug = [
    `Thank you for calling ${name}. How can we help you today?`,
    `Thanks for calling ${name}! Please stay on the line and we’ll connect you right away.`,
    `You’ve reached ${name}. We’re glad you called — one moment while we route you to the right place.`,
  ];
  const closedSug = [
