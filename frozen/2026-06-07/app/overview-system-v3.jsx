...
function FwdEditor({ d, patch, name, maxPlan=false, notif, patchNotif, slackConnected, slackChannel, onConnectSlack }){
  ...
  return (
    <React.Fragment>
      <label className="ps-toggle ps-fwdtoggle"><span><b>Forwarding</b><small>{d.fwdOn===false?'Off — calls go straight to voicemail.':'On — calls ring the destinations below.'}</small></span><button role="switch" aria-checked={d.fwdOn!==false} className={`toggle${d.fwdOn!==false?' on':''}`} onClick={()=>patch({fwdOn:d.fwdOn===false})}/></label>
