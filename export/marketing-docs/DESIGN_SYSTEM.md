...
- **Reference pages are living law, and reference is a role, not a roster.** Every content type has exactly one reference page (the current set is named in the header and maintained in PAGE_BLUEPRINTS.md); a content type without a reference page is not yet canon. New contributors learn the system by reading this document and the reference pages together. When implementation and documentation disagree, resolve it immediately, in whichever direction is right, and record the resolution; tolerated drift is how design systems die.
