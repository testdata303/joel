...
- **Headings:** sentence case. On Learn and use-case pages, prefer headings that are the reader's actual question ("What does a toll-free number cost?"); first-person is welcome where the reader's query is first-person ("Customers text my personal phone"). Never label-only headings ("Overview", "Benefits").
...
- **Definitions are canonical.** Use the pillar definition from `KNOWLEDGE_GRAPH.md` verbatim on first mention. One definition per entity, everywhere, forever.
...
- Write the page someone wishes already existed.
- Be the page someone bookmarks, shares, and comes back to - not just the page that ranks.
- Teach first; sell by explaining.
- Respect the reader's time - they're running a business between paragraphs.
- Earn trust before asking for action.
- Show the setup, not the concept.
- Say it once, say it plainly, link to the rest.
- Create pages worth citing.
- Every page strengthens the brand: helpful, specific, calm.
- If Numberline disappeared tomorrow, the page should still be worth reading.
- Write for the business owner at 10 p.m., not the marketer at 10 a.m.
