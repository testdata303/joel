...
2. Follow the approved template for the page's Content Type (see `templates/`). If none exists yet, use the highest-quality existing page of that type as the reference implementation (e.g. `for/plumbers.html` for industry pages). Match the site's visual vocabulary exactly - `assets/site.css`, function color palette, root-relative links (`/for/...`, `/learn/...`), `../`-relative assets.
...
9. Update the footer only within its caps: columns are Product · Features · Industries · Use cases · Learn · Company, ~8 curated links each, "See all" pointing to the hub index. Hubs absorb growth; the footer and nav never do.
...
- Nav is five items forever: How it works · Features · Pricing · Learn · Sign in + CTA. Nothing else ever enters it.
...
- Pillar canonical pages grow an "In this topic" section listing all children as children ship - pillars, not folders, are the graph's real hubs.
